...
export async function GET(req: NextRequest) {
  ...
  if (!session) return NextResponse.json({ error: "ไม่พบ session กรุณาเข้าสู่ระบบใหม่" }, { status: 401 });
  ...
  const { data: student } = await supabase
    .from("students").select("id, classroom_id").eq("id", studentId).maybeSingle();
  ...
  if (error) return NextResponse.json({ error: "ดึงตารางเรียนไม่สำเร็จ" }, { status: 500 });
