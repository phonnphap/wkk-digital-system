...
export default function StudentPortalClient({ studentId }: StudentPortalClientProps) {
  ...
  const fetchAssignments = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/student-portal/assignments?student_id=${studentId}`);
  ...
  const fetchGrades = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/student-portal/grades?student_id=${studentId}`);
    const data = await res.json();
      ...
      {tab === "attendance" && !loading && attendance && (
        <div>
          <div className="mb-4 grid grid-cols-4 gap-2 text-center text-sm">
            ...
            <div className="p-2 bg-blue-50 rounded">ลา {attendance.summary.excused}</div>
          </div>
