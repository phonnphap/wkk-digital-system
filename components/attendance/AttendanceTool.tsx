    ...
    <div className="rounded-2xl overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-3 px-5 pt-5 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-m italic mt-1">ตารางเช็คชื่อสำหรับคาบเรียนนี้</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => { setShowSwapDate(v => !v); setSwapError(""); }}
            className="shrink-0 px-3 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-black text-m flex items-center gap-1.5 transition-colors"
          >
            🔄 สลับคาบวันเรียน
          </button>
          <button
            onClick={() => setShowNoteCol(v => !v)}
            className="shrink-0 px-3 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-600 font-black text-m flex items-center gap-1.5 transition-colors"
          >
      ...
      {showSwapDate && (
        <div className="mx-5 mt-3 rounded-xl border-2 border-indigo-200 bg-indigo-50 px-4 py-3">
          <p className="text-m font-black text-indigo-700 mb-2">🔄 สลับคาบวันเรียน (เรียนชดเชย / สลับคาบฉุกเฉิน)</p>

          <div className="flex items-center gap-2 flex-wrap mb-3">
            <div className="flex items-center gap-1.5">
              <span className="text-m font-bold text-slate-500">จาก</span>
              <span className="px-2.5 py-1 rounded-lg bg-white border-2 border-slate-200 text-m font-black text-slate-700">
                {formatThaiDate(date)}
              </span>
            </div>
            <span className="text-slate-400 font-black">→</span>
            <div className="flex items-center gap-1.5">
              <span className="text-m font-bold text-slate-500">เป็น</span>
              <input
                type="date"
                value={swapToDate}
                onChange={e => setSwapToDate(e.target.value)}
                className="border-2 border-slate-200 rounded-lg px-2.5 py-1.5 text-m font-bold focus:border-indigo-400 focus:outline-none"
              />
            </div>
          </div>

          <div className="mb-3">
            <p className="text-m font-bold text-slate-500 mb-1.5">เหตุผล</p>
            <div className="flex gap-2 flex-wrap">
              {(Object.keys(REASON_CONFIG) as SwapReason[]).map(r => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-m font-black border-2 transition-colors ${
                    swapReason === r
          ...
          <input
            ...
            className="w-full border-2 border-slate-200 rounded-lg px-3 py-2 text-m font-bold focus:border-indigo-400 focus:outline-none mb-3"
          />
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={swapping}
              className="px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-m disabled:opacity-50"
            >
              {swapping ? "⏳ กำลังสลับ..." : "✅ ยืนยันสลับวัน"}
            </button>
            <button
              onClick={() => { setShowSwapDate(false); setSwapToDate(""); setSwapNote(""); setSwapError(""); }}
              className="px-3 py-1.5 rounded-xl border-2 border-slate-200 text-slate-500 font-black text-m"
            >
              ยกเลิก
            </button>
          </div>
          {swapError && <p className="text-red-600 text-m font-bold mt-2">❌ {swapError}</p>}
        </div>
      )}

      {/* แจ้งเตือนถ้าคาบนี้ถูกย้าย */}
      {movedAway && (
        <div className="mx-5 mt-3 rounded-xl border-2 border-indigo-200 bg-indigo-50 px-4 py-2.5 text-m font-bold text-indigo-600 flex items-center justify-between gap-2 flex-wrap">
          <span>
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg border-2 border-indigo-200 text-indigo-500 font-black text-m shrink-0"
          >
            ยกเลิกการสลับ
          </button>
        </div>
      ...
      {movedIn && (
        <div className="mx-5 mt-3 rounded-xl border-2 border-indigo-200 bg-indigo-50 px-4 py-2.5 text-m font-bold text-indigo-600">
      ...
      <div className="px-5 mt-4">
        <span className="inline-block px-4 py-1.5 rounded-full bg-blue-500 text-white text-m font-black">Default</span>
      </div>

      {/* ช่วงเวลา (แสดงผลอย่างเดียว ยังไม่ส่งเข้า API) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-5 mt-4">
        <div>
          <p className="text-m font-bold text-slate-500 mb-1 flex items-center gap-1.5">📅 เลือกเวลาเริ่ม</p>
        ...
        <div>
          <p className="text-m font-bold text-slate-500 mb-1 flex items-center gap-1.5">🕐 เลือกเวลาจบ</p>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 px-5 mt-4">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 text-m font-black text-sky-600 bg-sky-50 px-3 py-1.5 rounded-full">
            <span className="w-2 h-2 rounded-full bg-sky-500" /> {markedCount} / {students.length} marked
          </span>
          {STATUS_ORDER.map(st => (
            <span key={st} className={`px-2.5 py-1 rounded-full text-m font-black ${STATUS_CONFIG[st].chipBg} ${STATUS_CONFIG[st].chipText}`}>
        ...
        <div className="flex gap-2">
          {referenceMap && (
            <button onClick={pullFromReference}
              className="px-3 py-1.5 rounded-xl border-2 border-blue-200 bg-blue-50 text-blue-700 font-black text-m">
              📥 ดึงจาก{referenceLabel}
            </button>
          )}
          {holidayInfo && (
  <div className="mx-5 mt-3 rounded-xl border-2 border-rose-200 bg-rose-50 px-4 py-2.5 text-m font-bold text-rose-600">
    📅 วันนี้เป็นวันหยุด: {holidayInfo.name} — ถ้าไม่มีเรียนชดเชย ไม่ต้องเช็คชื่อ
        ...
        <table className="w-full min-w-[640px] border-collapse">
          <thead>
            <tr className="bg-slate-50">
              <th className="text-left text-m font-black text-slate-500 tracking-wide px-5 py-3 sticky left-0 bg-slate-50">STUDENT</th>
              {STATUS_ORDER.map(st => (
                <th key={st} className="px-2 py-3">
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 text-m font-black ${STATUS_CONFIG[st].chipText} ${STATUS_CONFIG[st].chipBg} px-2.5 py-1 rounded-full hover:brightness-95 active:scale-95 transition-all cursor-pointer`}
                  >
                    <span className={`w-2 h-2 rounded-full ${STATUS_CONFIG[st].dot}`} /> {STATUS_CONFIG[st].label}
                  </button>
                </th>
              ))}
              {showNoteCol && <th className="text-left text-m font-black text-slate-500 tracking-wide px-5 py-3">NOTE</th>}
            </tr>
          </thead>
          <tbody>
            {students.map((s, i) => {
                  ...
                  {showNoteCol && (
                    <td className="px-5 py-3">
                      <input
                        ...
                        className="w-full min-w-[140px] border-2 border-slate-200 rounded-lg px-2.5 py-1.5 text-m font-bold focus:border-sky-400 focus:outline-none"
                      />
                    </td>
                  )}
                </tr>
      ...
      {error && <p className="text-red-600 text-m font-bold bg-red-50 border-2 border-red-200 rounded-xl px-5 py-2 mx-5 mt-3">❌ {error}</p>}
